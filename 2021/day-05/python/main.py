...
print(f"part 1 answer = {len([point.danger for point in points if point.danger > 1])}")
...
for idx, line in enumerate(lines[:-1]):
    ...
    for point in [p for p in points if ((max(x1, x2) >= p.x >= min(x1, x2)) and (max(y1, y2) >= p.y >= min(y1, y2)))]:
        if x1 == x2 == point.x:
            ...
            if point.x <= max(x1, x2) and point.x >= min(x1, x2):
                point.inc_danger()
        elif (abs(x1 - point.x) == abs(y1 - point.y)):
            point.inc_danger()

print(f"part 2 answer = {len([point.danger for point in points if point.danger > 1])}")
